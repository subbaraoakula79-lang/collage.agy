...
async function runTest() {
    try {
        ...
        const api1 = axios.create({ baseURL: 'http://localhost:5001/api', headers: { Authorization: `Bearer ${token1}` } });
        ...
        const api4 = axios.create({ baseURL: 'http://localhost:5001/api', headers: { Authorization: `Bearer ${token4}` } });
        ...
        const selectedPg = pgCourses.slice(0, 3).map(c => c.id);
        await api4.post('/student/apply', { courseIds: selectedPg, admissionType: 'PG' });
